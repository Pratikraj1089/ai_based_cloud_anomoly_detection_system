...
from config import CPU_HIGH,CPU_DANGER,RAM_DANGER,DISK_HIGH,LOAD_HIGH,NET_SPIKE_MODERATE,NET_SPIKE_HIGH,PROCESS_DROP_HIGH
...
def classify_root_cause(metric:dict, trend:dict, consecutive_ram_increases:int=0) -> str:
    ...
    ni5=trend.get('net_in_5min_avg',ni); lr=la/cc if cc>0 else 0
    if cpu>=CPU_HIGH and ni5>=NET_SPIKE_MODERATE: return 'Possible DDoS / Traffic Overload'
    if consecutive_ram_increases>=5 and rd>0: return 'Possible Memory Leak'
    if pd<=-PROCESS_DROP_HIGH: return 'Possible Service Crash'
    if no>=NET_SPIKE_MODERATE and no>ni*2: return 'Possible Data Exfiltration / Spam'
